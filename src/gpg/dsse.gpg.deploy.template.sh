...
# file name: dsse.gpg.deploy.template.sh
...
usage() {
    ...
    # shellcheck disable=SC2059 # this is intentional
    printf "${Cyan}./$(basename "$0")${Yellow} --file${Color_Off} mftuser.dsse.gpg.info.json ${Yellow} --environment${Color_Off} fahrschule${Yellow} --action${Color_Off} delete\n"
    # shellcheck disable=SC2059 # this is intentional
    printf "${Cyan}./$(basename "$0")${Yellow} --file${Color_Off} mftuser.dsse.gpg.info.json ${Yellow} --environment${Color_Off} fahrschule\n"
    # shellcheck disable=SC2059 # this is intentional
    printf "${Cyan}./$(basename "$0")${Yellow} --file${Color_Off} dsse.gpg.info.json ${Yellow} --environment${Color_Off} fahrschule${Yellow} --node ${Color_Off}ctm-lin.trybmc.local\n"
}
...
while getopts ":a:f:n:p:r:z:h" OPTION; do
    case "${OPTION}" in
    a)
        GPG_KEY_ACTION="${OPTARG}"
        ;;
    f)
...
function display_project_details() {
    ...
    printf "${CYAN} - GPG Template File   :${RESET}${BLUE} %s${RESET}\n" "${GPG_KEY_TEMPLATE_PATH}"
    printf "${CYAN} - GPG Recipient       :${RESET}${BLUE} %s${RESET}\n" "${GPG_KEY_RECIPIENT}"
    printf " %s\n" "${SEPARATOR}"
    printf "${CYAN} - GPG Binary Local    :${RESET}${BLUE} %s${RESET}\n" "${GPG_PATH}"
    printf "${CYAN} - GPG Binary CTM Agent:${RESET}${BLUE} %s${RESET}\n" "${GPG_BIN_PATH}"
    printf "${CYAN} - GPG Action          :${RESET}${BLUE} %s${RESET}\n" "${GPG_KEY_ACTION}"
    ...
    # printf "${CYAN} - GPG Info File Path  :${RESET}${BLUE} %s${RESET}\n" "${GPG_KEY_IMPORT_PATH_INFO}"
    ...
    printf "${CYAN} - GPG UID Attribute   :${RESET}${BLUE} %s${RESET}\n" "${GPG_UID_ATTRIBUTE}"
    printf " %s\n" "${SEPARATOR}"
    printf "${CYAN} - GPG Key Status       :${RESET}${BLUE} %s${RESET}\n" "${GPG_KEY_STATUS}"
    printf "${CYAN} - CTM Template Status  :${RESET}${BLUE} %s${RESET}\n" "${CTM_PGP_STATUS}"
    printf " %s\n" "${SEPARATOR}"
...
GPG_KEY_IMPORT_FILE_INFO_PATH=$(dirname "${GPG_KEY_IMPORT_FILE_INFO}")
GPG_KEY_IMPORT_FILE_INFO_NAME=$(basename "${GPG_KEY_IMPORT_FILE_INFO}")
GPG_KEY_IMPORT_NAME=$(echo "${GPG_KEY_IMPORT_FILE_INFO_NAME}" | cut -d'.' -f1)

# Compute OpenGPG file path and working dir
if [[ "${GPG_KEY_IMPORT_FILE_INFO}" == *"/"* ]]; then
    GPG_KEY_WORKING_FOLDER="${GPG_KEY_IMPORT_FILE_INFO_PATH}"
    GPG_KEY_IMPORT_PATH_INFO="${GPG_KEY_IMPORT_FILE_INFO}"
else
    GPG_KEY_WORKING_FOLDER="${WORKING_DIR}/gpg/${GPG_KEY_IMPORT_NAME}"
    GPG_KEY_IMPORT_PATH_INFO="${GPG_KEY_WORKING_FOLDER}/${GPG_KEY_IMPORT_FILE_INFO_NAME}"
fi
...
if [[ -f "${GPG_KEY_IMPORT_PATH_INFO}" ]]; then
    ...
    # # Encrypt the file
    # # gpg -v --batch --yes --recipient vscheith --passphrase ******* --output uat/data.json.gpg --encrypt uat/data.json

    # # Decrypt the file
    # # gpg -v --batch --yes --passphrase ******* --pinentry-mode loopback --output uat/decrypted/data.json -d uat/data.json.gpg
    ...
    # CTM secrets not yet supported
    # create_json_file_adv "${CTM_GPG_TEMPLATE_NAME}" "${GPG_BIN_PATH}" "${CTM_SECRET_NAME}" "${GPG_KEY_RECIPIENT}" "${GPG_ATTRIBUTES_ENCRYPTION}" "${GPG_ATTRIBUTES_DECRYPTION}" "${GPG_KEY_TEMPLATE_PATH}"
    ...
    API_RESULT=$(ctm config secret::delete "${CTM_SECRET_NAME}" -e "${CTM_ENV}" 2>&1)
    log_with_timestamp "${AWS_PROJECT_LOG_LEVEL}" "${API_RESULT}"

    API_ACTION="ctm config secret::add"
    printf " ${LIGHT_BLUE} > API Action   : ${RESET}${Red} %s${RESET}\n" "${API_ACTION}"
    API_RESULT=$(ctm config secret::add "${CTM_SECRET_NAME}" "${GPG_KEY_PASSPHRASE}" -e "${CTM_ENV}" 2>&1)
    log_with_timestamp "${AWS_PROJECT_LOG_LEVEL}" "${API_RESULT}"
    ...
    # printf "${CYAN}  - GPG Key    : ${RESET}${Yellow} %s${RESET}\n" "${GPG_KEY_ID_TEMP}"
    ...
    for CTM_AGENT in "${CTM_AGENT_TEMP[@]}"; do
        ...
        API_RESULT=$(ctm config server:agent:mft:configuration::get "${CTM_SERVER}" "${CTM_AGENT}" -e "${CTM_ENV}" 2>/dev/null | jq -r '.pgpTempDir')
        log_with_timestamp "${PROJECT_LOG_LEVEL}" "${API_RESULT}"
        ...
        if test -n "${CTM_CMD_RESULT}"; then
            ...
            printf " ${LIGHT_BLUE} > API Action   : ${RESET}${Red} %s${RESET}\n" "${API_ACTION}"
            API_RESULT=$(ctm config server:agent:mft:pgptemplates::get "${CTM_SERVER}" "${CTM_AGENT}" -s name="${CTM_GPG_TEMPLATE_NAME}" -e "${CTM_ENV}")
            log_with_timestamp "${PROJECT_LOG_LEVEL}" "${API_RESULT}"
            CTM_CMD_RESULT="${API_RESULT}"
            ...
            if [[ "${CTM_PGP_TEMPLATE_COUNT}" -ge 1 && "${GPG_KEY_ACTION}" != "delete" ]]; then
                ...
                API_RESULT=$(ctm config server:agent:mft:pgptemplate::update "${CTM_SERVER}" "${CTM_AGENT}" "${CTM_GPG_TEMPLATE_NAME}" -f "${GPG_KEY_TEMPLATE_PATH}" -e "${CTM_ENV}" 2>&1)
                ...
                if [[ "${API_RESULT_MESSAGE}" == *"successfully"* ]]; then
                    CTM_PGP_STATUS="OK"
                    printf "\n${CYAN}  - CTM Status   : ${RESET}${DARK_GREY} %s:${Green} %s${RESET}\n" "${CTM_NOTES}" "${CTM_PGP_STATUS}"
                else
                    CTM_PGP_STATUS="Error"
                    printf "\n${CYAN}  - CTM Status   : ${RESET}${DARK_GREY} %s:${Red} %s${RESET}\n" "${CTM_NOTES}" "${CTM_PGP_STATUS}"
                fi
                ...
                API_RESULT=$(ctm config server:agent:mft:pgptemplate::add "${CTM_SERVER}" "${CTM_AGENT}" "${CTM_GPG_TEMPLATE_NAME}" -f "${GPG_KEY_TEMPLATE_PATH}" -e "${CTM_ENV}" 2>&1)
                API_RESULT_MESSAGE=$(echo "${API_RESULT}" | jq -r 'if has("message") then .message elif has("errors") then .errors[0].message else empty end')
                log_with_timestamp "${PROJECT_LOG_LEVEL}" "${API_RESULT_MESSAGE}"
                ...
                API_RESULT=$(ctm config server:agent:mft:pgptemplate::delete "${CTM_SERVER}" "${CTM_AGENT}" "${CTM_GPG_TEMPLATE_NAME}" -e "${CTM_ENV}" 2>&1)
                API_RESULT_MESSAGE=$(echo "${API_RESULT}" | jq -r 'if has("message") then .message elif has("errors") then .errors[0].message else empty end')
                log_with_timestamp "${PROJECT_LOG_LEVEL}" "${API_RESULT_MESSAGE}"
                ...
                log_with_timestamp "${PROJECT_LOG_LEVEL}" "${CTM_NOTES}"
                printf "${CYAN}  - CTM Notes    : ${RESET}${Green} %s${RESET}\n" "${CTM_NOTES}"
                CTM_PGP_STATUS="N/A"
            fi
        fi
    done
