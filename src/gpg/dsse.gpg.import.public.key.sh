...
# file name: dsse.gpg.import.public.key.sh
...
function display_project_details() {
    ...
    printf "${CYAN} - GPG Priv Status      :${RESET}${BLUE} %s${RESET}\n" "${GPG_KEY_STATUS_PRIVATE}"
    printf " %s\n" "${SEPARATOR}"
...
if [[ -f "${GPG_KEY_IMPORT_PATH_PUBLIC}" ]]; then
    ...
    # UPDATED_DATA=$(echo "${GPG_KEY_IMPORT_INFO}" | jq --arg keyid "${GPG_KEY_ID_PUBLIC}" '. += {"keyid": $keyid}')
    UPDATED_DATA=$(echo "${GPG_KEY_IMPORT_INFO}" | jq --arg keyid "${GPG_KEY_ID_PUBLIC}" '.keyid += {"public": $keyid}')
    echo -e " ${Cyan} - Data File    : ${Color_Off}${AWS_PROJECT_DATA_CONFIG_FILE}"
    # shellcheck disable=SC2034 # this is intentionals
    CMD_RESULT=$(echo "${UPDATED_DATA}" | tee "${GPG_KEY_IMPORT_PATH_INFO}")
