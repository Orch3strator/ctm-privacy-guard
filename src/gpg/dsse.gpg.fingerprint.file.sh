...
# file name: dsse.gpg.fingerprint.file.sh
...
usage() {
    ...
    # shellcheck disable=SC2059 # this is intentionals
    printf "${Cyan}./$(basename "$0")${Yellow} --file${Color_Off} /tmp/zzm/data.log.gpg.asc\n"
    # shellcheck disable=SC2059 # this is intentionals
    printf "${Cyan}./$(basename "$0")${Yellow} --file${Color_Off} /tmp/zzm/data.log.gpg.asc\n"
}
...
function display_project_details() {
    ...
    printf "${CYAN} - GPG Key Namwe        :${RESET}${BLUE} %s${RESET}\n" "${GPG_KEY_NAME}"
    printf "${CYAN} - GPG E-Mail           :${RESET}${BLUE} %s${RESET}\n" "${GPG_KEY_EMAIL}"
    printf "${CYAN} - GPG UID Label        :${RESET}${BLUE} %s${RESET}\n" "${GPG_UID_LABEL}"
    ...
    if [[ "${GPG_KEY_STATUS}" == "TRUE" ]]; then
        printf "${CYAN} - GPG Status           :${RESET}${Green} %s${RESET}\n" "${GPG_KEY_STATUS}"
    else
        printf "${CYAN} - GPG Status           :${RESET}${Red} %s${RESET}\n" "${GPG_KEY_STATUS}"

    fi

    if [[ "${GPG_FILE_EXISTS}" == "TRUE" ]]; then
        printf "${CYAN} - File Exists          :${RESET}${Green} %s${RESET}\n" "${GPG_FILE_EXISTS}"
    else
        printf "${CYAN} - File Exists          :${RESET}${Red} %s${RESET}\n" "${GPG_FILE_EXISTS}"
...
# shellcheck disable=SC2034 # this is intentionals
DATA_FOLDER_DECRYPTED="${DATA_FOLDER}/decrypted"
...
if [ -f "${DATA_PATH_IN}" ]; then
    ...
    API_RESULT=$(md5sum "${GPG_FILE_IN}" | awk -F' ' '{print $1}')
    log_with_timestamp "${PROJECT_LOG_LEVEL}" "${API_RESULT}"
    CHECKSUM_MD5="${API_RESULT}"

    API_ACTION="${CHECKSUM_SHA_CMD}"
    printf " ${LIGHT_BLUE} > API Action   : ${RESET}${Red} %s${RESET}\n" "${API_ACTION}"
    API_RESULT=$(${CHECKSUM_SHA_CMD} "${GPG_FILE_IN}" | awk -F' ' '{print $1}')
    log_with_timestamp "${PROJECT_LOG_LEVEL}" "${API_RESULT}"
    CHECKSUM_SHA512="${API_RESULT}"

    API_ACTION="Fingerprint File"
    printf " ${LIGHT_BLUE} # Stage       -> ${RESET}${LIGHT_CYAN} %s${RESET}\n" "${API_ACTION}"
    log_with_timestamp "INFO" "Start"

    API_ACTION="gpg --batch --yes --list-packets --no-tty"
    printf " ${LIGHT_BLUE} > API Action   : ${RESET}${Red} %s${RESET}\n" "${API_ACTION}"
    API_RESULT=$(gpg --batch --yes --list-packets --no-tty "${GPG_FILE_IN}" 2>&1)
    log_with_timestamp "${PROJECT_LOG_LEVEL}" "${API_RESULT}"
    GPG_CMD_RESULT="${API_RESULT}"

    if [[ "${GPG_CMD_RESULT}" == *"processing message failed"* ]]; then
        GPG_KEY_STATUS="FALSE"
    else

        API_ACTION="grep, awk"
        API_RESULT=$(echo "${GPG_CMD_RESULT}" | grep 'keyid' | awk -F' ' '{print $NF}' | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//')
        log_with_timestamp "${PROJECT_LOG_LEVEL}" "${API_RESULT}"
        GPG_KEY_ID="${API_RESULT}"

        API_ACTION="Extract OpenGPG Key details"
        printf " ${LIGHT_BLUE} # Stage       -> ${RESET}${LIGHT_CYAN} %s${RESET}\n" "${API_ACTION}"
        log_with_timestamp "INFO" "Start"
